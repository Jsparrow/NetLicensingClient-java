 ...
 * {@link com.labs64.netlicensing.domain.entity.BaseEntity}.
 */
public abstract class BaseEntityImpl extends Visitable implements BaseEntity {
	 ...
	 * Property name that is included in the list can not be used as custom property
	 * name. The list is extended by each derived entity class until the final
	 * business entity.
	 *
	...
	public Form asRequestForm() {
		...
		asPropertiesMap().entrySet().forEach(prop -> prop.getValue().stream().filter(value -> value != null)
				.forEach(value -> form.param(prop.getKey(), value.toString())));
		return form;
	}
	...
	protected MultivaluedMap<String, Object> asPropertiesMap() {
		...
		if (properties != null) {
			properties.entrySet().forEach(lp -> map.add(lp.getKey(), lp.getValue()));
		}
		return map;
	}
	...
	protected String toString(final MultivaluedMap<String, Object> propMap) {
		...
		for (final String propKey : propMap.keySet()) {
			...
			if (propValue != null
					&& (!(propValue instanceof Collection<?>) || ((Collection<?>) propValue).size() > 0)) {
