...
public class RestProviderJersey extends AbstractRestProvider {
	...
	/**
	 * @param basePath base provider path
	 */
	...
	/*
	 * @see com.labs64.netlicensing.provider.RestProvider#call(java.lang.String,
	 * java.lang.String, java.lang.Object, java.lang.Class, java.util.Map)
	 */
	@Override
	...
	/**
	 * @param target target object, to which the authentication headers will be
	 *               added
	 * @param auth   an object providing the authentication info
	 */
	private void addAuthHeaders(final WebTarget target, final Authentication auth) {
		if (auth != null) {
			// see also https://jersey.java.net/documentation/latest/client.html, chapter
			// "Securing a Client"
	...
	/**
	 * Reads entity of given type from response. Returns null when the response has
	...
	private <RES> RES readEntity(final Response response, final Class<RES> responseType) throws RestException {
		...
		try {
			...
			if (response.getStatus() == Response.Status.NO_CONTENT.getStatusCode()
					|| ex.getCause() instanceof NoContentException) {
				return null;
			} else {
				...
				final String body = buffered
						? new StringBuilder().append(" '").append(response.readEntity(String.class))
								.append("' of type '").append(response.getMediaType()).append("'").toString()
						: "";
	...
	private class JerseyDefaultConfig implements RestProvider.Configuration {
		...
		public String getUserAgent() {
			return new StringBuilder().append("NetLicensing/Java ").append(System.getProperty("java.version"))
					.append(" (http://netlicensing.io)").toString();
		}
