 ...
 * <p>
 * Licenses always belong to a certain {@linkplain LicenseeService licensee}.
 * Licenses are cloned off the corresponding {@linkplain LicenseTemplateService
 * license templates} when a licensee obtains them. On validation request the
 * licenses that belong to a licensee are processed according to the licensing
 * models configured for the product modules, and the validation result is sent
 * back for further processing to the requesting side (which is typically a
 * vendor service or a program executed by the end user).
 * <p>
 * In simple cases there is no need to work with licenses directly via
 * NetLicensing API, as licenses are assigned either by a licensing model
 * automatically (e.g. evaluation licenses) or actively purchased by a licensee
 * via NetLicensing Shop. However, for some complex licensing models, there may
 * be a need to assign licenses to a licensee programmatically from the vendor
 * software.
 */
public class LicenseService {
	 ...
	 * 
	 * @param context               determines the vendor on whose behalf the call
	 *                              is performed
	 * @param licenseeNumber        parent licensee to which the new license is to
	 *                              be added
	 * @param licenseTemplateNumber license template that the license is created
	 *                              from
	 * @param transactionNumber     For privileged logins specifies transaction for
	 *                              the license creation. For regular logins new
	 *                              transaction always created implicitly, and the
	 *                              operation will be in a separate transaction.
	 *                              Transaction is generated with the provided
	 *                              transactionNumber, or, if transactionNumber is
	 *                              null, with auto-generated number.
	 * @param license               non-null properties will be taken for the new
	 *                              object, null properties will either stay null,
	 *                              or will be set to a default value, depending on
	 *                              property.
	...
	public static License get(final Context context, final String number) throws NetLicensingException {
		return NetLicensingService.getInstance().get(context,
				new StringBuilder().append(Constants.License.ENDPOINT_PATH).append("/").append(number).toString(), null,
				License.class);
	}
	 ...
	 * @return list of licenses (of all products) or null/empty list if nothing
	 *         found.
	 * @throws com.labs64.netlicensing.exception.NetLicensingException any subclass
	 *         of
	 *         {@linkplain com.labs64.netlicensing.exception.NetLicensingException}.
	 ...
	 * @param transactionNumber transaction for the license update. Created
	 *                          implicitly if transactionNumber is null. In this
	 *                          case the operation will be in a separate
	 *                          transaction.
	 * @param license           non-null properties will be updated to the provided
	...
	public static License update(final Context context, final String number, final String transactionNumber,
		...
		return NetLicensingService.getInstance().post(context,
				new StringBuilder().append(Constants.License.ENDPOINT_PATH).append("/").append(number).toString(), form,
				License.class);
	}
	 ...
	 * Deletes license.
	 * <p>
	 * When any license is deleted, corresponding transaction is created
	 * automatically.
	 * 
	 * @param context      determines the vendor on whose behalf the call is
	 *                     performed
	...
	public static void delete(final Context context, final String number, final boolean forceCascade)
		...
		NetLicensingService.getInstance().delete(context,
				new StringBuilder().append(Constants.License.ENDPOINT_PATH).append("/").append(number).toString(),
				params);
	}
