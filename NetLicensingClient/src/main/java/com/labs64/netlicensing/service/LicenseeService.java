 ...
 * <p>
 * Licensee is usually an end customer, capable of obtaining licenses for the
 * product. Each licensee is associated with a single {@linkplain ProductService
 * product}. From the vendor perspective a licensee object in NetLicensing may
 * correspond to a physical instance of the product, customer account within a
 * vendor's service, or it can represent a security dongle given to a user. In
 * practice, each licensee must only have a unique identifier associated with
 * it, that is communicated to NetLicensing as licensee number for performing
 * operations related to this licensee. Licensee doesn't need to have an own
 * account within NetLicensing. There are two main operations performed for
 * licensees: validation and obtaining new licenses. Validation process is
 * typically completely transparent to the end user and performed from the
 * vendor's product by means of this API. Licensee can be offered to obtain new
 * licenses for the product by redirecting the user to the NetLicensing Shop in
 * the web browser.
 */
public class LicenseeService {
	 ...
	 * @param licensee      non-null properties will be taken for the new object,
	 *                      null properties will either stay null, or will be set to
	 *                      a default value, depending on property.
	...
	public static Licensee get(final Context context, final String number) throws NetLicensingException {
		...
		return NetLicensingService.getInstance().get(context,
				new StringBuilder().append(Constants.Licensee.ENDPOINT_PATH).append("/").append(number).toString(),
				null, Licensee.class);
	}
	 ...
	 * @return list of licensees (of all products) or null/empty list if nothing
	 *         found.
	 * @throws com.labs64.netlicensing.exception.NetLicensingException any subclass
	 *         of
	 *         {@linkplain com.labs64.netlicensing.exception.NetLicensingException}.
	...
	public static Page<Licensee> list(final Context context, final String filter) throws NetLicensingException {
		...
		return NetLicensingService.getInstance().list(context, Constants.Licensee.ENDPOINT_PATH, params,
				Licensee.class);
	}
	 ...
	 * @param licensee non-null properties will be updated to the provided values,
	...
	public static Licensee update(final Context context, final String number, final Licensee licensee)
		...
		return NetLicensingService.getInstance().post(context,
				new StringBuilder().append(Constants.Licensee.ENDPOINT_PATH).append("/").append(number).toString(),
				licensee.asRequestForm(), Licensee.class);
	}
	 ...
	 *                     performed
	 * @param number       licensee number
	 * @param forceCascade if true, any entities that depend on the one being
	 *                     deleted will be deleted too
	 * @throws com.labs64.netlicensing.exception.NetLicensingException any subclass
	 *         of
	 *         {@linkplain com.labs64.netlicensing.exception.NetLicensingException}.
	 *         These exceptions will be transformed to the corresponding service
	...
	public static void delete(final Context context, final String number, final boolean forceCascade)
		...
		NetLicensingService.getInstance().delete(context,
				new StringBuilder().append(Constants.Licensee.ENDPOINT_PATH).append("/").append(number).toString(),
				params);
	}
	 ...
	 *                             performed
	 * @param number               licensee number
	 * @param productNumber        optional productNumber, must be provided in case
	 *                             licensee auto-create is enabled
	 * @param licenseeName         optional human-readable licensee name in case
	 *                             licensee will be auto-created
	 * @param validationParameters optional validation parameters, specific to
	 *                             licensing model. See licensing model
	 *                             documentation for details.
	 * @param meta                 optional parameter, receiving messages returned
	 *                             within response <infos> section.
	 * @throws com.labs64.netlicensing.exception.NetLicensingException any subclass
	 *         of
	 *         {@linkplain com.labs64.netlicensing.exception.NetLicensingException}.
	 *         These exceptions will be transformed to the corresponding service
	 ...
	 *                             performed
	 * @param number               licensee number
	 * @param validationParameters optional validation parameters. See
	 *                             ValidationParameters and licensing model
	 *                             documentation for details.
	 * @param meta                 optional parameter, receiving messages returned
	 *                             within response <infos> section.
	 * @throws com.labs64.netlicensing.exception.NetLicensingException any subclass
	...
	public static ValidationResult validate(final Context context, final String number,
		...
		if (validationParameters != null) {
			...
			for (final Entry<String, Map<String, String>> productModuleValidationParams : validationParameters
				...
				form.param((Constants.ProductModule.PRODUCT_MODULE_NUMBER + Integer.toString(pmIndex)),
						productModuleValidationParams.getKey());
				for (final Entry<String, String> param : productModuleValidationParams.getValue().entrySet()) {
					form.param((param.getKey() + Integer.toString(pmIndex)), param.getValue());
				}
				++pmIndex;
			}
		}
		return NetLicensingService.getInstance().post(context,
				new StringBuilder().append(Constants.Licensee.ENDPOINT_PATH).append("/").append(number).append("/")
						.append(Constants.Licensee.ENDPOINT_PATH_VALIDATE).toString(),
				form, ValidationResult.class, meta);
	}
	 ...
	 * @param context              determines the vendor on whose behalf the call is
	 *                             performed
	 * @param number               the number of the licensee receiving licenses
	 * @param sourceLicenseeNumber the number of the licensee delivering licenses
	 * @throws com.labs64.netlicensing.exception.NetLicensingException any subclass
	 *         of
	 *         {@linkplain com.labs64.netlicensing.exception.NetLicensingException}.
	 *         These exceptions will be transformed to the corresponding service
	...
	public static void transfer(final Context context, final String number, final String sourceLicenseeNumber)
		...
		NetLicensingService.getInstance().post(
				context, new StringBuilder().append(Constants.Licensee.ENDPOINT_PATH).append("/").append(number)
						.append("/").append(Constants.Licensee.ENDPOINT_PATH_TRANSFER).toString(),
				form, Licensee.class);
	}
