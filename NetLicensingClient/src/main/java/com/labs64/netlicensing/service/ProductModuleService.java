 ...
 * <p>
 * {@linkplain ProductService Product} may comprise of multiple product modules,
 * but must have at least one. Each product module is licensed using one of the
 * licensing models offered by NetLicensing service. Licensing within a product
 * module is independent of other product modules, however all product modules
 * of a single product are visible to every {@linkplain LicenseeService
...
public class ProductModuleService {
	 ...
	 *                      performed
	 * @param productNumber parent product to which the new product module is to be
	 *                      added
	 * @param productModule non-null properties will be taken for the new object,
	...
	public static ProductModule create(final Context context, final String productNumber,
		...
		return NetLicensingService.getInstance().post(context, Constants.ProductModule.ENDPOINT_PATH, form,
				ProductModule.class);
	}
	...
	public static ProductModule get(final Context context, final String number) throws NetLicensingException {
		...
		return NetLicensingService.getInstance().get(context,
				new StringBuilder().append(Constants.ProductModule.ENDPOINT_PATH).append("/").append(number).toString(),
				null, ProductModule.class);
	}
	 ...
	 * @return list of product modules (of all products) or null/empty list if
	 *         nothing found.
	 * @throws com.labs64.netlicensing.exception.NetLicensingException any subclass
	 *         of
	 *         {@linkplain com.labs64.netlicensing.exception.NetLicensingException}.
	...
	public static Page<ProductModule> list(final Context context, final String filter) throws NetLicensingException {
		...
		return NetLicensingService.getInstance().list(context, Constants.ProductModule.ENDPOINT_PATH, params,
				ProductModule.class);
	}
	 ...
	 * @param productModule non-null properties will be updated to the provided
