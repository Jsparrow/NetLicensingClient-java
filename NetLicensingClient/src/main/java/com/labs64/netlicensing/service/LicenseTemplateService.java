 ...
 * <p>
 * License template is a configuration element of a
 * {@linkplain ProductModuleService product module}. License templates define
 * concrete items available for a {@linkplain LicenseeService licensee} to
 * obtain. License template specifies what is an item, its price, amount (if
 * applicable), etc. When licensee obtains an item, actual
 * {@linkplain LicenseService license} is created off the corresponding license
 * template, and this license is assigned to the licensee. Thus, the item is
 * licensed for the Licensee.
 */
public class LicenseTemplateService {
	 ...
	 *                            performed
	 * @param productModuleNumber parent product module to which the new license
	 *                            template is to be added
	 * @param licenseTemplate     non-null properties will be taken for the new
	 *                            object, null properties will either stay null, or
	 *                            will be set to a default value, depending on
	 *                            property.
	...
	public static LicenseTemplate create(final Context context, final String productModuleNumber,
		...
		return NetLicensingService.getInstance().post(context, Constants.LicenseTemplate.ENDPOINT_PATH, form,
	...
	public static LicenseTemplate get(final Context context, final String number) throws NetLicensingException {
		...
		return NetLicensingService.getInstance().get(context, new StringBuilder()
				.append(Constants.LicenseTemplate.ENDPOINT_PATH).append("/").append(number).toString(), null,
				LicenseTemplate.class);
	}
	 ...
	 * @return list of license templates (of all products/modules) or null/empty
	...
	public static Page<LicenseTemplate> list(final Context context, final String filter) throws NetLicensingException {
		...
		return NetLicensingService.getInstance().list(context, Constants.LicenseTemplate.ENDPOINT_PATH, params,
				LicenseTemplate.class);
	}
	 ...
	 * @param licenseTemplate non-null properties will be updated to the provided
	...
	public static LicenseTemplate update(final Context context, final String number,
		...
		return NetLicensingService.getInstance().post(context, new StringBuilder()
				.append(Constants.LicenseTemplate.ENDPOINT_PATH).append("/").append(number).toString(),
	...
	public static void delete(final Context context, final String number, final boolean forceCascade)
		...
		NetLicensingService.getInstance().delete(context, new StringBuilder()
				.append(Constants.LicenseTemplate.ENDPOINT_PATH).append("/").append(number).toString(), params);
	}
